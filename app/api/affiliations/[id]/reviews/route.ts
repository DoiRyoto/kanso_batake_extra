...
export async function GET(
  ...
  const affiliationId = parseInt(params.id);
  if (!affiliationId) {
    return NextResponse.json({ error: "No affiliation ID" }, { status: 400 });
  }
  const searchParams = request.nextUrl.searchParams;
  const searchTag = searchParams.get("searchTag");

  try {
    const reviewDatas = await fetchReviews(searchTag, affiliationId);
    return NextResponse.json(reviewDatas, { status: 200 });
  } catch (error) {
    console.error(error);
    return NextResponse.json(
      {
        error: `Failed to fetch reviews posted by users belonging the affliation with ID = ${affiliationId}.`,
      },
      { status: 500 },
    );
  }
